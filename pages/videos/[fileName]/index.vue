<template>
  <v-container fluid>
    ...
    <v-breadcrumbs :items="['VIPPO', 'Videos', $route.params.fileName]" bg-color="indigo-lighten-5"
      class="text-body-2"></v-breadcrumbs>
    <!-- End: Page Breadcrumbs -->
    <!-- Page Toolbar -->
    <v-toolbar color="background" class="text-primary mt-4">
      <v-toolbar-title class="ml-1 text-h5"><v-icon icon="mdi-play-box-outline" size="small"></v-icon>
        Video Playback
      </v-toolbar-title>
    </v-toolbar>
    <!-- End: Page Toolbar -->
      ...
      <v-col>
        <v-card color="black">
          <div :style="{
            ...
            <video ref="videoElement" :src="video.downloadPath" controls preload="metadata" :style="{
                position: 'absolute',
                top: 0,
                left: 0,
                width: '100%',
          ...
          <v-card-text>
            <v-window v-model="tabSummary">
              ...
              <v-window-item value="summary_transcript">
                <v-row justify="end">
                  <v-col>
                    <v-btn size="small" prepend-icon="mdi-auto-fix" @click="() => moveToNewSummarizationPage()">Create
                      custom
                      summary</v-btn>
                  </v-col>
                </v-row>
                <div style="width: 100%;height: 300px;">
                  <video controls style="object-fit: contain;width: 100%; height: 100%;"
                    src="https://storage.cloud.google.com/vippo-video-maker/podcast-podcast_auto_summarization.mp4/sd.mp4"></video>
                </div>
                ...
                <v-expansion-panels>
                  <v-expansion-panel v-for="(obj, index) in video.meta.labels" :key="index">
                    ...
                    <v-expansion-panel-text>
                      <v-chip-group>
                        <v-chip v-for="(timestamp, index) in obj.segments" :key="index" @click="this.$refs.videoElement.currentTime =
                          timestamp.startTime
                          ">{{ timestamp.startTime }} -
              ...
              <v-window-item value="objects">
                <v-expansion-panels>
                  <v-expansion-panel v-for="(obj, index) in video.meta.objects" :key="index">
                    ...
                    <v-expansion-panel-text>
                      <v-chip-group>
                        <v-chip v-for="(timestamp, index) in obj.timestamps" :key="index" @click="this.$refs.videoElement.currentTime =
                          timestamp.startTime
                          ">{{ timestamp.startTime }} -
                          {{ timestamp.endTime }}</v-chip>
                      </v-chip-group>
