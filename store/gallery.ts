...
export const actions: ActionTree<RooState, RooState> = {
  async fetchGallery(
    ...
    const videos: VideoDto[] = data.items.map((v: any) => ({
      name: v.name,
      url: v.src,
      hasError: false,
      ...
      videoAnalysis: mockVideoAnalysisData,
    }));
  ...
  async uploadVideo({ dispatch }, { video, signedURL, onProgressCallback }) {
    const { data } = await GalleryService.instance.uploadVideo(
      video,
      signedURL,
      onProgressCallback
    );
